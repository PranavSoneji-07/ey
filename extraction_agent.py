...
# OCR# extraction_agent.py

# Global variable to hold the initialized PaddleOCR object
PADDLE_OCR_INSTANCE = None
PADDLE_LANG = "en"


# ... (Keep get_nlp() as is if it uses the singleton pattern)
...
def get_ocr():
    ...
    if PADDLE_OCR_INSTANCE is None:
        print("\n--- LAZY LOADING PADDLE OCR MODEL ---")
        # The initialization must be inside the function now
        PADDLE_OCR_INSTANCE = PaddleOCR(lang=PADDLE_LANG)
...
def process_pdf(pdf_path: str) -> pd.DataFrame:
    ...
    if df.empty:
        # Return empty dictionary if no data was processed
        return {} 
